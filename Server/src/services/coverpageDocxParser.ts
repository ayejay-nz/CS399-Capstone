...
import { extractCoverpageBuffer } from '../utils/coverpageBufferExtractor';
...
export async function parseCoverpageDocx(file: Buffer | Blob): Promise<CoverpageDocx | Buffer> {
    try {
        ...
        // If coverpage parsing failed but we have appendices, extract coverpage buffer
        if (Buffer.isBuffer(coverpageResult)) {
            const coverpageBuffer = await extractCoverpageBuffer(
            ...
            if (appendices.length > 0) {
                return {
                    content: appendices,
                    unparsedCoverpageBuffer: coverpageBuffer,
                };
            } else {
