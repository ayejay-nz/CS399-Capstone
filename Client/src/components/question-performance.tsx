  ...
  const questions = React.useMemo<QuestionPerf[]>(() =>
    questionStats.map((qb) => {
      ...
      return {
        ...
        answerCounts: counts,
        answerOptions: opts,
        correctOptions: qb.correctAnswers.map(
  ...
  const pieData = selectedQuestion
    ? allLabels.map((letter, idx) => ({
        ...
        fill: selectedQuestion.correctOptions.includes(letter)
          ? "#10B981"
          : ["#EF4444", "#DC2626", "#B91C1C", "#991B1B", "#7F1D1B"][
              idx
            ],
      }))
    : [];
        ...
        {selectedQuestion ? (
          <div className="space-y-6">
            <div className="text-white">
              <h4 className="font-semibold mb-2">Question {selectedQuestion.questionId} Details:</h4>
              <p>
                <span className="font-medium">Question:</span> {selectedQuestion.questionText}
              </p>
              <p>
                <span className="font-medium">Marks:</span> {selectedQuestion.marks}
              </p>
              <p>
                <span className="font-medium">Percentage Correct:</span> {selectedQuestion.correctPct}%
              </p>
            </div>
            ...
            <div className="rounded-xl border border-[#27272A] bg-black p-6">
              ...
              <ChartContainer config={{ theme: "dark" }} className="mx-auto w-full max-w-[350px] h-[350px] [&_.recharts-pie-label-text]:fill-white">
                <PieChart width={350} height={350}>
                  <ChartTooltip content={<ChartTooltipContent />} />
                  <Pie data={pieData} dataKey="value" nameKey="name" outerRadius={120} label={{ fill: "#fff", fontSize: 12 }}>
            ...
            <div className="rounded-xl border border-[#27272A] bg-black p-4">
              ...
              <div className="flex justify-center">
                <RadialBarChart width={250} height={150} data={radialData} startAngle={180} endAngle={0} innerRadius={60} outerRadius={100} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                  <PolarRadiusAxis tick={false} tickLine={false} axisLine={false}>
                    <ChartLabel content={({ viewBox }) =>
                      viewBox?.cx != null && viewBox?.cy != null ? (
                        <text x={viewBox.cx} y={viewBox.cy} textAnchor="middle">
                          <tspan x={viewBox.cx} y={viewBox.cy - 8} className="fill-white text-2xl font-bold">
                  ...
                  <RadialBar dataKey="correct" stackId="a" cornerRadius={5} fill="#10B981" clockWise={false} className="stroke-transparent stroke-2" />
                  <RadialBar dataKey="incorrect" stackId="a" cornerRadius={5} fill="#EF4444" clockWise={false} className="stroke-transparent stroke-2" />
                </RadialBarChart>
              </div>
            </div>
          </div>
